  ...
  return (
    <motion.div className="relative">
      ...
      >
        ...
        <motion.div
          className="absolute inset-0 bg-gradient-to-r from-orange-800/20 to-amber-900/20 rounded-xl"
        ...
        <div className="relative flex items-center w-full">
          <span className={`flex relative ${active ? "text-white" : "text-orange-800 group-hover:text-orange-900"}`}>
            {link.icon}
            {hasNotification && (
              <motion.div
                initial={{ scale: 0 }}
      ...
      <AnimatePresence>
        {open && showSubItems && link.subItems && (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
            className="ml-4 mt-1 space-y-1 border-l-2 border-orange-200 pl-4"
          >
            {link.subItems.map((subItem) => (
              <motion.button
                ...
                whileHover={{ x: 2 }}
                onClick={() => onClick()}
                className="flex items-center p-2 rounded-lg w-full text-sm text-orange-700 hover:bg-orange-50 hover:text-orange-800 transition-all duration-200"
              >
                {subItem.label}
              </motion.button>
            ))}
      ...
      <AnimatePresence>
        {showTooltip && (
          <motion.div
            ...
            className="absolute left-full ml-3 top-1/2 -translate-y-1/2 bg-orange-950 text-white shadow-xl rounded-lg px-3 py-2 text-sm font-medium whitespace-nowrap z-50 border border-orange-800"
          >
            <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 bg-orange-950 rotate-45 border-l border-b border-orange-800" />
            {link.label}
          </motion.div>
        )}
      </AnimatePresence>
...
const Logo = ({ open }: { open: boolean }) => (
  <Link href="/" className="flex items-center px-2 py-6 group">
    <motion.div
      className="h-10 w-10 rounded-xl bg-gradient-to-br from-orange-800 via-amber-900 to-orange-950 flex items-center justify-center shadow-lg relative overflow-hidden"
    ...
    {open && (
      <motion.div
        ...
        className="ml-3"
      >
        <h1 className="text-xl font-bold text-orange-900 group-hover:text-orange-800 transition-colors">
          Brew & Bliss
        </h1>
  ...
  >
    <motion.button
      ...
      className={`
        ...
        ${active
          ? "bg-gradient-to-r from-orange-900 to-amber-900 text-white shadow-lg"
    ...
    >
      <div className="relative">
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-200 via-amber-300 to-orange-400 flex items-center justify-center shadow-md">
          <IconUser className="w-4 h-4 text-orange-900" />
        </div>
      ...
      {open && (
        ...
        >
          ...
          <p className="text-xs text-orange-700">Premium Member</p>
        </motion.div>
      )}
    </motion.button>
    ...
    <AnimatePresence>
      {!open && isHovered && (
        <motion.div
          ...
          animate={{ opacity: 1, x: 0, scale: 1 }}
          exit={{ opacity: 0, x: 20, scale: 0.9 }}
          className="absolute left-full ml-3 top-1/2 -translate-y-1/2 bg-orange-950 text-white shadow-xl rounded-lg px-3 py-2 text-sm font-medium whitespace-nowrap z-50"
        >
          <div className="absolute -left-1 top-1/2 -translate-y-1/2 w-2 h-2 bg-orange-950 rotate-45" />
          <div>
            <p className="font-medium">John Doe</p>
            <p className="text-xs text-orange-300">Premium Member</p>
          </div>
        </motion.div>
      )}
    </AnimatePresence>
    ...
    <>
      ...
      <motion.aside
        ...
        className={`
          ...
          bg-gradient-to-b from-orange-50 via-amber-50/50 to-orange-50
      ...
      >
        {/* Header */}
        <div className="flex-shrink-0 p-4 border-b border-orange-200/50">
          <div className="flex justify-between items-center">
            <Logo open={open} />
            <motion.button
              onClick={toggleSidebar}
              className="p-2 rounded-full hover:bg-orange-100/50 text-orange-800 transition-colors duration-200"
